...
![停車中のシアンの照明が審査を通過し、赤い点滅も停車中なら許可され、走り出した瞬間に実行時保証モニタが実行中のまま止め、60km/hで再要求すると今度はバスに1フレームも流れない様子](docs/assets/demo.gif)

*上のGIFには4つのビートがあります。どれも仕込みではなく、ライブデモそのものです。*

1. **停車中。** 落ち着いたシアンのプログラムが審査を通り、足元とドアが灯ります — `PASS 4`
2. **同じく停車中**、「全部を赤く速く点滅」は*許可されます*。ルールは状況依存であって、
   一律の禁止ではありません — `PASS 7`
3. **走り出します。** そのプログラムは既に承認され、既にバスへ流れています。実行時保証モニタが
   ECUにラッチ済みの状態を再審査し、**実行中のまま**止めます — `RTA intervention ×6`
4. **60km/hで同じ要求を出すと、今度は最初から拒否され**、バスに1フレームも届きません — `REJECT 6`

生き残るのはダッシュボードだけです（減光され、点滅から呼吸へ格下げ）。運転者の前方視野に
入る唯一のゾーンなので、破棄ではなくクランプされます。
...
![同じパイプラインを Unity の車内シーンから動かし、デバッグHUDに監査証跡を表示している様子](docs/assets/demo-unity.gif)
