...
![停車中は「全部を赤く速く点滅」が審査を通って車内全体が光り、走り出すと実行時保証モニタが実行中のまま止め、60km/hで再度要求すると今度はバスに1フレームも流れない様子](docs/assets/demo.gif)

*上のGIFでは3つのことが起きています。どれも仕込みではなく、ライブデモそのものです。*

1. **停車中**、「全部を赤く速く点滅」は審査を通過します。7ゾーンすべてが点灯 — `PASS 7`
2. **走り出します。** プログラムは既に承認され、既にバスへ流れています。そこで実行時保証モニタが
   ECUにラッチ済みの状態を再審査し、実行中のまま止めます（バスに `RTA intervention` が6件）。
   速度が上がるにつれて車内が暗くなります。
3. **60km/hで同じ意図をもう一度**投げると、今度はバスに1フレームも届きません — `REJECT 6`
